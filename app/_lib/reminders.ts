...
import { computeNetBalances, counterpartiesForGroup } from './balances';
import type { ActionResult } from './context';
import { getContext, now } from './context';
import { newId } from './ids';
import { requireGroupMember, runGuarded } from './membership';
...
export async function sendReminderAction(
  ...
  return runGuarded(async () => {
    ...
    const [group] = await db
      .select({ name: groups.name, simplifyDebts: groups.simplifyDebts })
      .from(groups)
    ...
    const counterparties = counterpartiesForGroup(
      { ...ledger, simplifyDebts: group.simplifyDebts, netBalances: computeNetBalances(ledger) },
      myMemberId,
    );
    ...
    if (lastReminder && nowTs - lastReminder.sentAt < REMINDER_COOLDOWN_SECONDS) {
      return {
        ok: false,
        error: 'You already sent a reminder to this member in the last 24 hours.',
      };
    }
